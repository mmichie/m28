...
func Init_REParserModule() *core.DictValue {
	...
	module.SetStr("parse", core.NewNamedBuiltinFunction("parse", func(args []core.Value, ctx *core.Context) (core.Value, error) {
		...
		state := core.NewDict()
		state.SetStr("flags", flags)
		state.SetStr("str", pattern)
		state.SetStr("groups", core.NumberValue(1)) // At least 1 group (the whole match)
		state.SetStr("groupdict", core.NewDict())
		state.SetStr("grouprefpos", core.NewDict())
		state.SetStr("groupwidths", core.NewDict())
		...
		subPattern.SetStr("state", state)
		subPattern.SetStr("data", core.NewList()) // Empty list of pattern operations
		subPattern.SetStr("width", core.TupleValue{core.NumberValue(0), core.NumberValue(65535)})

		// Add getwidth method
		core.DebugLog("[RE-PARSER] Adding getwidth method\n")
		subPattern.SetStr("getwidth", core.NewNamedBuiltinFunction("getwidth", func(getwidthArgs []core.Value, getwidthCtx *core.Context) (core.Value, error) {
	...
	module.SetStr("parse_template", core.NewNamedBuiltinFunction("parse_template", func(args []core.Value, ctx *core.Context) (core.Value, error) {
		if len(args) < 2 {
